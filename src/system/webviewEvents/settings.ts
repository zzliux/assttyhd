...
export default function webviewSettigns() {
    ...
    webview.on("getSettings").subscribe(([_param, done]) => {
        ...
        //     type: 'assttyhd_setting_launch_after_boot',
        //     //@ts-ignore
        ...
        ret = [...ret, {
            ...
            type: 'assttyhd_setting_floaty_debugger_draw',
            ...
            desc: 'OCR扩展',
            name: 'ocr_extend',
            type: 'assttyhd_setting_ocr_extend',
    ...
    webview.on("saveSetting").subscribe(([item, done]) => {
        if ('autojs_inner_setting' === item.type) {
        ...
        } else if ('assttyhd_setting' === item.type) {
            let storeSettings = storeCommon.get('settings', {});
            ...
            toastLog('保存成功');
        } else if ('assttyhd_setting_floaty_debugger_draw' === item.type) {
            if (item.enabled) {
                drawFloaty.init();
            } else {
                drawFloaty.destroy();
        ...
        } else if ('assttyhd_setting_ocr_extend' === item.type) {
            let storeSettings = storeCommon.get('settings', {});
            if (item.enabled) {
                let ocr = null;
        ...
        } else if ('assttyhd_setting_launch_after_boot' === item.type) {
            if (item.enabled) {
                //@ts-ignore
